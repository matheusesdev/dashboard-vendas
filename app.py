...
def download_xlsx():
    try:
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            def write_pivot(df_pivot, title, start_row):
                sheet_pivots.merge_range(start_row, 0, start_row, len(df_pivot.columns), title, subtitle_format)
                df_pivot.to_excel(writer, sheet_name='Tabelas Dinâmicas', startrow=start_row + 1)
                return start_row + len(df_pivot) + 4
            
            pivot_empreendimento = df.pivot_table(index='EMPREENDIMENTO', columns='SITUAÇÃO', values='CONTADOR', aggfunc='sum', fill_value=0, margins=True, margins_name='Total Geral')
            current_row = write_pivot(pivot_empreendimento, 'Status por Empreendimento', current_row)

            pivot_tipologia = df.pivot_table(index='TIPOLOGIA', columns='SITUAÇÃO', values='CONTADOR', aggfunc='sum', fill_value=0, margins=True, margins_name='Total Geral')
            current_row = write_pivot(pivot_tipologia, 'Status por Tipologia', current_row)

            sheet_pivots.set_column('A:A', 45)
            sheet_pivots.set_column('B:Z', 18)

            # --- NOVA ABA: Tipologia por Etapa (Status) ---
            df_status = df.copy()
            df_status['SITUAÇÃO'] = df_status['SITUAÇÃO'].replace({
                'VENDIDO': 'VENDIDA', 'DISPONIVEL': 'DISPONÍVEL', 'BLOQUEADO': 'BLOQUEADA'
            })
            
            if not df_status.empty:
                pivot_etapa_tipologia = pd.pivot_table(
                    df_status,
                    index='ETAPA',
                    columns=['TIPOLOGIA', 'SITUAÇÃO'],
                    values='CONTADOR',
                    aggfunc='sum',
                    fill_value=0
                )
                if not pivot_etapa_tipologia.empty:
                    pivot_etapa_tipologia.to_excel(writer, sheet_name='Tipologia x Etapa (Status)')
                    worksheet_tip_etapa = writer.sheets['Tipologia x Etapa (Status)']
                    worksheet_tip_etapa.set_column('A:A', 20)
                    worksheet_tip_etapa.set_column('B:Z', 15)

            # --- Aba Final: Dados Consolidados ---
            df_final = df.drop(columns=['CONTADOR'], errors='ignore')
